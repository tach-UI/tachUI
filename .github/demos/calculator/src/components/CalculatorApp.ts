...
export function CalculatorApp(): ComponentInstance {
    ...
    const tapeEntriesSignal: Signal<TapeEntry[]> = appState.wrappedValue.tapeEntriesSignal;
    const tapeVisibleSignal: Signal<boolean> = appState.wrappedValue.tapeDisplaySignal;
    const result = VStack({
        spacing: 0,
        children: [
            HStack({
                ...
                children: [
                    // App Logo
                    Image(tachuiLogo).size({ height: 64 }).build(),
                    Text("tachulator")
                        .font({
