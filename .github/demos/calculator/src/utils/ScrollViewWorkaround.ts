...
export function directReactiveChildrenPattern(items: () => string[]): ComponentInstance {
    ...
    return ScrollView({
        children: createComputed(() => items().map(item => Text(item).padding(8).build())),
        direction: "vertical"
    })
        .frame({ height: 120 })
...
export function workingScrollViewWithForEach(items: () => string[]): ComponentInstance {
    ...
    //     ForEach({
    //       items: items,
    //       children: (item: string) => Text(item).padding(8).build()
    //     })
    //   ],
    //   direction: "vertical"
    // })
    ...
    return Text("ForEach pattern - see comments in code").padding(8).build();
}
/**
...
export function workingScrollViewWithVStack(items: () => string[]): ComponentInstance {
    ...
    const reactiveVStack = createComputed(() => {
        ...
        const children = currentItems.map((item, index) => Text(`${index + 1}. ${item}`)
            .padding(8)
...
export function workingScrollViewWithManualUpdate(): ComponentInstance {
    ...
    const scrollContent = items().map((item, index) => Text(`${index + 1}. ${item}`)
        .padding(8).build());
    return ScrollView({
