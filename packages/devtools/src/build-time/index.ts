 ...
 */
export function configureBuildTimeValidation(
  config: Partial<BuildTimeValidationConfig>
): void {
  ...
  test(): void {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        throw new Error(
          'Build-time validation cannot run in browser environment'
        )
      }
      ...
      console.info(
        ...
        buildTools.map((t: any) => `${t.name} v${t.version}`).join(', ')
      )
      console.info(
        '🎯 Primary build tool:',
        primaryTool ? `${primaryTool.name} v${primaryTool.version}` : 'None'
      )
  ...
  getStats() {
    if (typeof window !== 'undefined') {
      return {
        buildTime: {
          available: false,
          error: 'Cannot run in browser environment',
        },
      }
    }
