...
import { OptimizedPluginErrorHandler } from '../../src/plugins/simplified-error-handler'
import { PluginError } from '../../src/plugins/simplified-types'
import type { TachUIPlugin, TachUIInstance } from '../../src/plugins/simplified-types'
...
describe('Week 3: Runtime Performance Optimization', () => {
  describe('OptimizedLazyPluginLoader', () => {
    ...
    test('caches loaded plugins for performance', async () => {
      ...
      vi.spyOn(loader as any, 'doLoadPlugin')
  ...
  describe('OptimizedPluginErrorHandler', () => {
    ...
    test('circuit breaker pattern prevents excessive error handling', () => {
      ...
      const circuitStatus = errorHandler.getCircuitBreakerStatus('install:error-test-plugin') as any
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Circuit breaker open for plugin error-test-plugin')
      )
    ...
    test('error metrics tracking and aggregation', () => {
      ...
      expect(stats.errorsByType['runtime-error']).toBe(1)
    })
    ...
    test('error resolution and circuit breaker reset', () => {
      ...
      let circuitStatus = errorHandler.getCircuitBreakerStatus('install:error-test-plugin') as any
      expect(circuitStatus.isOpen).toBe(true)
      ...
      circuitStatus = errorHandler.getCircuitBreakerStatus('install:error-test-plugin') as any
      expect(circuitStatus.isOpen).toBe(false)
      expect(circuitStatus.failureCount).toBe(0)
      
      // Error metrics should show resolution
      const metrics = errorHandler.getErrorMetrics('install:error-test-plugin') as any
    ...
    test('performance impact measurement', () => {
      ...
      const errors = Array.from({ length: 100 }, () => new Error('Performance test'))
