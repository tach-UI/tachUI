...
describe('Grid Animation System', () => {
  ...
  describe('GridCSSGenerator Animation Methods', () => {
    it('should generate layout change CSS', () => {
      ...
      expect(css.transition).toContain('grid-template-columns 300ms ease-out 100ms')
      expect(css.transition).toContain('grid-template-rows 300ms ease-out 100ms')
    })
    ...
    it('should generate responsive change CSS', () => {
      ...
      expect(css.transition).toContain('grid-template-columns 250ms ease-in-out')
    ...
    it('should generate item animation keyframes', () => {
      const fadeKeyframes = GridCSSGenerator.generateItemAnimationKeyframes('enter', 'fade')
      const scaleKeyframes = GridCSSGenerator.generateItemAnimationKeyframes('exit', 'scale')
      
      expect(fadeKeyframes['@keyframes grid-item-enter-fade']).toContain('opacity: 0')
      expect(fadeKeyframes['@keyframes grid-item-enter-fade']).toContain('opacity: 1')
      expect(scaleKeyframes['@keyframes grid-item-exit-scale']).toContain('scale(1)')
      expect(scaleKeyframes['@keyframes grid-item-exit-scale']).toContain('scale(0.8)')
    })

    it('should generate slide animation keyframes', () => {
      const slideUpKeyframes = GridCSSGenerator.generateItemAnimationKeyframes('enter', 'slide-up')
      const slideDownKeyframes = GridCSSGenerator.generateItemAnimationKeyframes('exit', 'slide-down')
      
      expect(slideUpKeyframes['@keyframes grid-item-enter-slide-up']).toContain('translateY(20px)')
      expect(slideUpKeyframes['@keyframes grid-item-enter-slide-up']).toContain('translateY(0)')
      expect(slideDownKeyframes['@keyframes grid-item-exit-slide-down']).toContain('translateY(0)')
      expect(slideDownKeyframes['@keyframes grid-item-exit-slide-down']).toContain('translateY(20px)')
    })
  })
  ...
  describe('GridAnimations Preset Functions', () => {
    ...
    it('should create scale-in animation config', () => {
      ...
      expect(config.itemChanges?.enter?.easing).toBe('cubic-bezier(0.34, 1.56, 0.64, 1)')
    })
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle invalid animation types gracefully', () => {
      expect(() => {
        GridCSSGenerator.generateItemAnimationKeyframes('enter', 'invalid' as any)
      }).not.toThrow()
    })
