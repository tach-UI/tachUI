...
describe('BackgroundModifier with State Support', () => {
  ...
  describe('state-based gradient support', () => {
    ...
    it('should change background on hover', () => {
      ...
      const mouseenterCall = addEventListener.mock.calls.find(
    ...
    it('should return to default on mouse leave', () => {
      ...
      const modifier = new BackgroundModifier({
        background: noAnimationGradients,
      })
      modifier.apply({} as any, mockContext)

      // Get handlers
      const mouseenterHandler = addEventListener.mock.calls.find(
        call => call[0] === 'mouseenter'
      )![1]
    ...
    it('should handle active state correctly', () => {
      ...
      const mousedownHandler = addEventListener.mock.calls.find(
  ...
  describe('state transition logic', () => {
    ...
    it('should prioritize disabled state', () => {
      mockElement.hasAttribute = vi
        .fn()
        .mockImplementation(attr => attr === 'disabled')
    ...
    it('should prioritize focus state over default', () => {
      mockElement.matches = vi
        .fn()
        .mockImplementation(selector => selector === ':focus')
