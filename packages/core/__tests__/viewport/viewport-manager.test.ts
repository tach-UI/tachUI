...
import { detectViewportEnvironment } from '../../src/viewport/platform-detection'
import type { ViewportAdapter, ViewportInstance, WindowConfig } from '../../src/viewport/types'
import {
  getViewportManager,
  setViewportManager,
  TachUIViewportManager,
} from '../../src/viewport/viewport-manager'
...
describe('TachUIViewportManager', () => {
  ...
  describe('Capability Detection', () => {
    ...
    it('should prefer native windows when requested', () => {
      const windowType = manager.getOptimalWindowType({ preferNativeWindow: true })
  ...
  describe('Window Options', () => {
    it('should handle window options', async () => {
      ...
      const window = await manager.openWindow('test-window', mockComponent, options)
