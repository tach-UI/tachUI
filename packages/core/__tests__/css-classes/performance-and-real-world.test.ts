...
import { VStack, HStack } from '../../src/components/wrapper'
...
describe('CSS Classes Enhancement - Performance and Real-World Usage', () => {
  describe('Performance Benchmarks', () => {
    ...
    it('should handle complex class strings efficiently', () => {
      const manager = new CSSClassManager()
      const complexClasses = 'bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 text-white font-bold py-2 px-4 rounded-full shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 ease-in-out'
  ...
  describe('Real-World Usage Scenarios', () => {
      ...
      const navbar = HStack({
        ...
        children: [
          ...
          HStack({
            ...
            children: [
              Text('Home', { css: 'text-gray-900 hover:text-gray-700 px-3 py-2 text-sm font-medium' }),
              Text('Products', { css: 'text-gray-500 hover:text-gray-700 px-3 py-2 text-sm font-medium' }),
              Text('About', { css: 'text-gray-500 hover:text-gray-700 px-3 py-2 text-sm font-medium' })
            ]
          }),
    ...
    it('should handle a dashboard with multiple widgets', () => {
      const createWidget = (title: string, value: string, trend: 'up' | 'down' | 'neutral') => {
        const trendColor = trend === 'up' ? 'text-green-600' : trend === 'down' ? 'text-red-600' : 'text-gray-600'
        
        return VStack({
          css: 'bg-white rounded-lg shadow p-6 border border-gray-200',
          children: [
            Text(title, { css: 'text-sm font-medium text-gray-500 uppercase tracking-wide' }),
            Text(value, { css: 'text-3xl font-bold text-gray-900 mt-2' }),
            Text(trend === 'up' ? '↑ 12%' : trend === 'down' ? '↓ 5%' : '→ 0%', { 
  ...
  describe('Framework Integration Performance', () => {
    it('should handle Tailwind CSS utility classes efficiently', () => {
      const tailwindClasses = [
        // Layout
        'flex', 'flex-col', 'grid', 'grid-cols-12', 'gap-4', 'space-y-2',
        // Sizing
        'w-full', 'h-screen', 'min-h-0', 'max-w-md', 'mx-auto',
        // Colors
        'bg-blue-500', 'text-white', 'border-gray-300', 'ring-blue-500',
        // Typography
        'text-lg', 'font-bold', 'leading-tight', 'tracking-wide',
        // Effects
        'shadow-lg', 'rounded-lg', 'opacity-75', 'transform', 'scale-105',
        // Interactive
        'hover:bg-blue-600', 'focus:ring-2', 'transition-colors', 'duration-200'
      ]
    ...
    it('should handle mixed framework classes without conflicts', () => {
      const mixedClasses = [
        ...
        // Bootstrap
        'btn', 'btn-primary', 'mb-3', 'd-flex', 'justify-content-center',
        // Custom
        'custom-component', 'theme-dark', 'animation-fade-in'
      ]
