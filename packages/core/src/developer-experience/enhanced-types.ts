/**
 * Enhanced TypeScript Type Definitions (Phase Maroon Week 2)
 *
 ...
 */
export type StrictComponentFactory<T extends StrictComponentProps = StrictComponentProps> = (
  props: T
) => ComponentInstance & {
  readonly props: T
...
export interface EnhancedSignalFactory {
  ...
  <T = undefined>(
    name?: string
  ): [EnhancedSignal<T | undefined>, (value: T | ((prev: T | undefined) => T)) => void]
}
...
export interface TypedEffect {
  <T extends readonly EnhancedSignal<any>[]>(
    dependencies: T,
    effect: (
      ...values: { [K in keyof T]: T[K] extends EnhancedSignal<infer U> ? U : never }
    ) => undefined | (() => void)
  ): () => void
