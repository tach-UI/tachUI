 ...
 * ```
 */
export declare function createEnhancedEffect(effectFn: () => undefined | (() => void), options?: EffectOptions): () => void;
/**
 * Create an effect with high priority for user interactions
 */
export declare function createHighPriorityEffect(effectFn: () => undefined | (() => void), options?: Omit<EffectOptions, 'priority'>): () => void;
/**
 * Create an effect with low priority for background tasks
 */
export declare function createLowPriorityEffect(effectFn: () => undefined | (() => void), options?: Omit<EffectOptions, 'priority'>): () => void;
/**
 * Create an effect that only runs when dependencies change (not immediately)
 */
export declare function createWatchEffect(effectFn: () => undefined | (() => void), options?: Omit<EffectOptions, 'fireImmediately'>): () => void;
/**
 * Create an effect with automatic error recovery
 */
export declare function createResilientEffect(effectFn: () => undefined | (() => void), onError?: (error: Error, attempt: number) => boolean, // return true to retry
maxRetries?: number, options?: EffectOptions): () => void;
/**
 * Batch multiple effect creations for performance
 */
export declare function createEffectBatch(effects: Array<{
    fn: () => undefined | (() => void);
