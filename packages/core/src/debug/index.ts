...
class DebugManager {
  ...
  private removeDebugStyles(): void {
    ...
    document.querySelectorAll('.tachui-debug').forEach(el => {
      el.classList.remove('tachui-debug', 'tachui-debug-labeled', 'tachui-debug-bounds')
    })
  }
}
...
export const enableDebug = (options?: Partial<DebugConfig>) => debugManager.enable(options)
