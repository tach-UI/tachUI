...
import type { ComponentValidator } from './comprehensive'
 ...
 */
export function registerComponentValidator(validator: ComponentValidator): void {
 ...
 */
export function unregisterComponentValidator(packageName: string, componentName: string): void {
 ...
 */
export function getComponentValidator(packageName: string, componentName: string): ComponentValidator | undefined {
  const key = `${packageName}:${componentName}`
...
export function registerPluginValidation(config: PluginValidationConfig): void {
  ...
  for (const component of components) {
    const validator: ComponentValidator = {
      ...
      validate: component.validate || createDefaultValidator(packageName, component)
    }
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.info(`🔍 Registered ${components.length} validators for package: ${packageName}`)
  }
}
 ...
 */
function createDefaultValidator(packageName: string, component: ComponentValidationDefinition) {
  return (args: unknown[]) => {
    // Import TachUIValidationError dynamically to avoid circular dependency
    const { TachUIValidationError } = require('./comprehensive')

    if (component.requiredProps?.length && args.length === 0) {
      throw new TachUIValidationError(`${component.name} component requires props`, {
    ...
    if (args.length > 0) {
      ...
      if (!props || typeof props !== 'object') {
        throw new TachUIValidationError(`${component.name} requires a props object`, {
          component: component.name,
          package: packageName,
      ...
      if (component.requiredProps) {
        ...
        for (const requiredProp of component.requiredProps) {
          if (propsObj[requiredProp] === undefined) {
            throw new TachUIValidationError(`${component.name} ${requiredProp} property is required`, {
              component: component.name,
              package: packageName,
...
export function getValidationStats(): {
  ...
  const packages = Array.from(packageValidators.entries()).map(([name, components]) => ({
    name,
 ...
 */
export function validateComponent(packageName: string, componentName: string, args: unknown[]): void {
 ...
 */
export function hasValidator(packageName: string, componentName: string): boolean {
