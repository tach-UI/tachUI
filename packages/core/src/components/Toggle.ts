...
export class EnhancedToggle implements ComponentInstance<ToggleProps> {
  ...
  render() {
    const { labelPosition = 'trailing', spacing = 8, accessibilityHint } = this.props
    ...
    const children =
      labelPosition === 'leading' && label ? [label, control] : label ? [control, label] : [control]
...
export const ToggleStyles = {
   ...
   */
  Switch(isOn: boolean | Signal<boolean>, props: Omit<ToggleProps, 'isOn' | 'variant'> = {}) {
   ...
   * Checkbox toggle
   */
  Checkbox(isOn: boolean | Signal<boolean>, props: Omit<ToggleProps, 'isOn' | 'variant'> = {}) {
   ...
   * Button toggle
   */
  Button(isOn: boolean | Signal<boolean>, props: Omit<ToggleProps, 'isOn' | 'variant'> = {}) {
   ...
   * Small toggle
   */
  Small(isOn: boolean | Signal<boolean>, props: Omit<ToggleProps, 'isOn' | 'size'> = {}) {
   ...
   * Large toggle
   */
  Large(isOn: boolean | Signal<boolean>, props: Omit<ToggleProps, 'isOn' | 'size'> = {}) {
...
export const ToggleUtils = {
  ...
  batch: {
     ...
     */
    toggleAll(
      toggles: Array<{ isOn: boolean | Signal<boolean>; onToggle?: (isOn: boolean) => void }>,
      newState: boolean
    ) {
