 ...
 */
export class EnhancedImage extends ComponentWithCSSClasses implements ComponentInstance<ImageProps>, Concatenatable<ImageProps> {
  ...
  constructor(public props: ImageProps) {
    ...
    const [loadingStateSignal, setLoadingState] = createSignal<ImageLoadingState>(initialState)
    ...
    useLifecycle(this, {
      onDOMReady: (_elements, primaryElement) => {
        ...
        if (this.props.src instanceof ImageAsset && primaryElement instanceof HTMLImageElement) {
          this.setupImageAssetReactivityForDOMElement(this.props.src, primaryElement)
        }
      }
    })
  ...
  render() {
    ...
    if (loadingState === 'loading' && this.props.placeholder) {
      ...
      } else if (this.props.placeholder && typeof this.props.placeholder === 'object') {
    ...
    if (loadingState === 'error' && this.props.errorPlaceholder) {
      if (typeof this.props.errorPlaceholder === 'string') {
        ...
        return [errorElement]
      } else if (this.props.errorPlaceholder && typeof this.props.errorPlaceholder === 'object') {
        return this.props.errorPlaceholder.render()
      }
    }

    // Resolve ImageAsset to initial src value, reactive updates handled separately
    const initialSrc = this.props.src instanceof ImageAsset
   ...
   */
  private setupImageAssetReactivityForDOMElement(imageAsset: ImageAsset, domElement: HTMLImageElement): void {
  ...
  concat<U extends Concatenatable<any>>(other: U): ConcatenatedComponent<ImageProps | U> {
    ...
    const metadata: ConcatenationMetadata = {
      ...
      accessibilityRole: other instanceof ConcatenatedComponent
        ? this.mergeAccessibilityRoles('group', other.metadata.accessibilityRole)
  ...
  private mergeAccessibilityRoles(
    ...
    if (thisRole === 'composite' || existingRole === 'composite') return 'composite'
    return 'group'
  }
}
