/**
 * Development Tools System (Phase 3.2.2)
 *
...
import { type ComponentMetrics, globalPerformanceMonitor } from './performance'
...
export class DevTools {
  ...
  constructor() {
    ...
    const [treeSignal, setTree] = createSignal<Map<string, ComponentTreeNode>>(new Map())
  ...
  enable(): void {
    ...
    globalPerformanceMonitor.configure({
      ...
      trackReactive: true,
      trackMemory: this.config.trackMemoryUsage,
      trackRender: true,
    })
  ...
  registerComponent(componentId: string, instance: ComponentInstance, parentId?: string): void {
    ...
    const metrics = globalPerformanceMonitor.getComponentMetrics().get(componentId)
    ...
    const treeNode: ComponentTreeNode = {
      ...
      depth,
      metrics,
      reactiveSubscriptions: this.getReactiveSubscriptions(componentId),
      contextProviders: this.getContextProviders(componentId),
      contextConsumers: this.getContextConsumers(componentId),
      renderCount: metrics?.updateCount || 0,
      lastRenderTime: metrics?.lastRenderDuration || 0,
  ...
  unregisterComponent(componentId: string): void {
    ...
    if (treeNode) {
      ...
      this.addDebugEvent({
        ...
        componentId,
        componentName: treeNode.name,
        data: { lifetime: performance.now() - (treeNode.metrics?.mountTime || 0) },
      })
    }
  ...
  getRootComponents(): ComponentTreeNode[] {
    return Array.from(this.componentTree.values()).filter((node) => node.parent === null)
  }
  ...
  getPerformanceSummary(): {
    ...
    const totalRenderTime = components.reduce((sum, c) => sum + c.lastRenderTime, 0)
    ...
    return JSON.stringify(
      {
        ...
        performanceMetrics: globalPerformanceMonitor.exportMetrics(),
        timestamp: Date.now(),
      },
      null,
      2
  ...
  private getComponentName(instance: ComponentInstance): string {
    ...
    // Fallback to id or generic name
    return instance.id || 'Component'
  }

  private getReactiveSubscriptions(_componentId: string): string[] {
    // This would need to be implemented with reactive system integration
    return []
  }

  private getContextProviders(_componentId: string): string[] {
    // This would need to be implemented with context system integration
    return []
  }

  private getContextConsumers(_componentId: string): string[] {
    // This would need to be implemented with context system integration
    return []
  }
  ...
  private updateComponentMetrics(): void {
    ...
    const performanceMetrics = globalPerformanceMonitor.getComponentMetrics()

    for (const [componentId, treeNode] of this.componentTree) {
      const metrics = performanceMetrics.get(componentId)
      if (metrics) {
        treeNode.metrics = metrics
        treeNode.renderCount = metrics.updateCount
        treeNode.lastRenderTime = metrics.lastRenderDuration
        treeNode.memoryUsage = metrics.memoryUsage

        // Check for performance warnings
        if (metrics.lastRenderDuration > 16) {
          this.addWarning(componentId, `Slow render: ${metrics.lastRenderDuration.toFixed(2)}ms`)
        }

        if (metrics.memoryUsage > 1024 * 1024) {
          // 1MB
          this.addWarning(
            componentId,
...
export const devUtils = {
  ...
  logComponentTree(): void {
    ...
    function logNode(node: ComponentTreeNode, indent = 0): void {
      ...
      console.log(`${prefix}${node.name}#${node.id}${renderTime}${warnings}${errors}`)
