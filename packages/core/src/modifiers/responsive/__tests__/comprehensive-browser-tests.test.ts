...
import { ResponsiveDevTools, BrowserCompatibility, useResponsiveInspector } from '../dev-tools'
import { 
...
import { OptimizedCSSGenerator, ResponsivePerformanceMonitor } from '../performance'
...
describe('Comprehensive Browser Compatibility Tests', () => {
  ...
  describe('Browser Feature Detection', () => {
    ...
    test('detects container queries support', () => {
      ...
      expect(mockCSS.supports).toHaveBeenCalledWith('container-type', 'inline-size')
    })
  })
  ...
  describe('Cross-Browser Responsive Behavior', () => {
    Object.entries(mockBrowserEnvironments).forEach(([browserName, config]) => {
      test(`works correctly in ${browserName}`, () => {
        ...
        mockCSS.supports.mockImplementation((property: string, value: string) => {
          if (property === 'display' && value === 'grid') return config.supports.cssGrid
          if (property === 'display' && value === 'flex') return config.supports.flexbox
          if (property === '--test') return config.supports.customProperties
          if (property === 'width' && value === '100vw') return config.supports.viewportUnits
          if (property === 'container-type') return config.supports.containerQueries
          return false
        })
  ...
  describe('Responsive Data Management Edge Cases', () => {
    ...
    test('handles very large datasets', () => {
      ...
      const pagination = ResponsiveDataUtils.createResponsivePagination(largeData, {
        base: 50,
        md: 100
      })
