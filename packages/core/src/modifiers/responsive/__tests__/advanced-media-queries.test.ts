...
describe('Advanced Media Query Features', () => {
  ...
  describe('Color Scheme Preferences', () => {
    ...
    test('MediaQueries contains correct color scheme queries', () => {
      ...
      expect(MediaQueries.noColorSchemePreference).toBe('(prefers-color-scheme: no-preference)')
    })
  })
  ...
  describe('Accessibility Preferences', () => {
    ...
    test('MediaQueries contains correct accessibility queries', () => {
      ...
      expect(MediaQueries.allowMotion).toBe('(prefers-reduced-motion: no-preference)')
      expect(MediaQueries.highContrast).toBe('(prefers-contrast: high)')
      expect(MediaQueries.lowContrast).toBe('(prefers-contrast: low)')
      expect(MediaQueries.normalContrast).toBe('(prefers-contrast: no-preference)')
    })
  })
  ...
  describe('Resolution Queries', () => {
    test('MediaQueries contains resolution-based queries', () => {
      ...
      expect(MediaQueries.standardRes).toBe('(min-resolution: 120dpi) and (max-resolution: 192dpi)')
      expect(MediaQueries.highRes).toBe('(min-resolution: 192dpi)')
    })
    
    test('MediaQueries custom resolution builder works', () => {
      expect(MediaQueries.customResolution(150)).toBe('(min-resolution: 150dpi)')
      expect(MediaQueries.customResolution(300)).toBe('(min-resolution: 300dpi)')
    })
  })
  ...
  describe('Aspect Ratio Queries', () => {
    ...
    test('MediaQueries custom aspect ratio builders work', () => {
      ...
      expect(MediaQueries.minAspectRatio('16/10')).toBe('(min-aspect-ratio: 16/10)')
  ...
  describe('Media Query Utilities', () => {
    test('combineMediaQueries() combines queries with AND', () => {
      ...
      expect(combined).toBe('(min-width: 768px) and (orientation: landscape) and (prefers-color-scheme: dark)')
    })
  ...
  describe('Custom Media Query Builder', () => {
    test('custom media query builders work correctly', () => {
      ...
      expect(MediaQueries.between(768, 1024)).toBe('(min-width: 768px) and (max-width: 1024px)')
      
      expect(MediaQueries.minHeight(600)).toBe('(min-height: 600px)')
      expect(MediaQueries.heightBetween(400, 800)).toBe('(min-height: 400px) and (max-height: 800px)')
    })
  })
  ...
  describe('Data Preferences', () => {
    test('MediaQueries contains data preference queries', () => {
      ...
      expect(MediaQueries.allowData).toBe('(prefers-reduced-data: no-preference)')
      expect(MediaQueries.reduceTransparency).toBe('(prefers-reduced-transparency: reduce)')
      expect(MediaQueries.allowTransparency).toBe('(prefers-reduced-transparency: no-preference)')
    })
  })
  ...
  describe('Update and Scripting Capabilities', () => {
    test('MediaQueries contains update and scripting queries', () => {
      ...
      expect(MediaQueries.scriptingInitialOnly).toBe('(scripting: initial-only)')
    })
  })
  ...
  describe('Device-Specific Patterns', () => {
    test('MediaQueries contains device-specific queries', () => {
      ...
      expect(MediaQueries.iPad).toBe('(min-width: 768px) and (max-width: 1024px)')
      expect(MediaQueries.desktopSmall).toBe('(min-width: 1024px) and (max-width: 1440px)')
      expect(MediaQueries.desktopLarge).toBe('(min-width: 1440px)')
      
      expect(MediaQueries.keyboardNavigation).toBe('(hover: none) and (pointer: coarse)')
    })
  })
})