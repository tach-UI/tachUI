 ...
 */
export function detectViewportEnvironment(config: PlatformDetectionConfig = {}): ViewportEnvironment {
...
function isMobile(): boolean {
  ...
  return (
    isTouchOnly || (hasSmallScreen && isMobileUserAgent) || hasCoarsePointer || hasSmallViewport
  )
}
 ...
 */
function detectCapabilities(platform: ViewportEnvironment['platform'], config: PlatformDetectionConfig): ViewportCapabilities {
  ...
  switch (platform) {
    ...
    case 'web':
      return {
        ...
        crossWindowCommunication: canUseBroadcastChannel() || canUseMessageChannel(),
...
export function createCapabilityChecker(environment: ViewportEnvironment) {
  return {
    canOpenWindow: (preferNative = false) => {
      ...
      return environment.capabilities.multiWindow || environment.capabilities.modalOverlays
    },
