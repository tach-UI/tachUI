...
import { withComponentContext } from '../runtime/component-context'
import { createRoot } from '../reactive/context'
...
export interface WindowGroupProps<T = any> extends WindowOptions {
  ...
  children: T extends undefined ? () => ComponentInstance : (data: T) => ComponentInstance
...
export function WindowGroup<T = undefined>(props: WindowGroupProps<T>): WindowGroupComponent<T> {
  const manager = getViewportManager()
  ...
  return {
    ...
    async open(data?: T): Promise<void> {
      if (data !== undefined && props.for) {
        // Data-driven window
        await group.openWindow(data, props.children as (data: T) => ComponentInstance)
      } else {
        // Simple window
...
export interface WindowGroupComponent<T = any> {
  ...
  id: string
  title: string
  content: T extends undefined ? () => ComponentInstance : (data: T) => ComponentInstance
  options: WindowOptions
  group: IWindowGroup
...
export const WindowUtils = {
   ...
   */
  paletteWindow(id: string, title: string, content: () => ComponentInstance): WindowSceneComponent {
    return Window({
      id,
      title,
      width: 250,
