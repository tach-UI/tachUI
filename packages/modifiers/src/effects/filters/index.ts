 ...
 * ```
 */
export function opacity(value: number): FilterModifier {
  return new FilterModifier({ filter: { opacity: value } })
}
 ...
 * ```
 */
export function backdropFilter(config: FilterConfig | string): FilterModifier {
  return new FilterModifier({ filter: config })
}
 ...
 * ```
 */
export function filterDropShadow(shadow: string): FilterModifier {
