...
import { isSignal, isComputed, createEffect, getThemeSignal } from '@tachui/core/reactive'
...
export class TextShadowModifier extends BaseModifier<TextShadowOptions> {
  ...
  private generateTextShadowCSS(config: ReactiveTextShadowConfig): string {
    const x = this.resolveValue(config.x, 0)
    const y = this.resolveValue(config.y, 0)
    // Support both 'blur' and 'radius' (prefer blur if present)
    const blur = this.resolveValue(config.blur !== undefined ? config.blur : config.radius, 0)
...
export function textShadow(
  config: ReactiveTextShadowConfig | ReactiveTextShadowConfig[] | string
): TextShadowModifier {
...
export function textOutline(
  ...
  const shadows: ReactiveTextShadowConfig[] = []
