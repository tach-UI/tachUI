...
export class ProgrammaticNavigationPath extends NavigationPath {
  private _destinationRegistry: Map<
    string,
    (value: any) => NavigationDestination
  > = new Map()
  ...
  registerDestination<T>(
    type: string,
    handler: (value: T) => NavigationDestination
  ): void {
  ...
  handleDeepLink(url: string): any {
    ...
    if (handler) {
      ...
      if (pathParts.length > 1) {
        params.id = pathParts[1] // For simple cases like /profile/123
      }
...
export class NavigationPersistenceManager {
  ...
  private _stateMap: Map<string, any> = new Map()

  constructor(
    storageKey: string = 'tachui-navigation',
  ...
  /**
   * Save navigation state with key
   *
  ...
  saveState(key: string, state: any): void {
    ...
    try {
      const allStates = Object.fromEntries(this._stateMap)
      localStorage.setItem(this._storageKey, JSON.stringify(allStates))
    } catch (error) {
  ...
  /**
   * Load navigation state by key
   *
   * @param key - Storage key
   * @returns Loaded state or null
   */
  loadState(key: string): any | null {
    return this._stateMap.get(key) || null
  }

  /**
   * Get navigation state by key (alias for loadState)
   *
   * @param key - Storage key
  ...
  /**
   * Clear specific navigation state
   *
  ...
  isValidState(state: any): boolean {
    ...
    return typeof state.path === 'string' && typeof state.timestamp === 'number'
  }

  /**
   * Save navigation state to storage (legacy API for NavigationContext)
   *
   * @param context - Navigation context to save
   */
  saveNavigationState(context: NavigationContext): void {
    if (typeof window === 'undefined') return
  ...
  /**
   * Restore navigation state from storage (legacy API)
   *
  ...
  restoreState(navigationId?: string): any | null {
    ...
    try {
      ...
      if (navigationId) {
        ...
        return (
          history.find(state => state.navigationId === navigationId) || null
        )
      } else {
...
export class NavigationAnimationManager {
  ...
  /**
   * End a named animation
   */
  endAnimation(name: string): void {
    this._activeAnimations.delete(name)
  }

  /**
   * Static method to create animation configuration
   */
  ...
  /**
   * Static method to validate animation configuration
   */
  static isValidConfig(config: any): boolean {
    if (!config || typeof config !== 'object') return false
    if (config.type && !['slide', 'fade', 'scale'].includes(config.type))
      return false
    if (
      config.duration &&
  ...
  /**
   * Static method to combine multiple animations
   */
  static combine(animations: any[]): any[] {
    return animations.filter(Boolean)
  }

  /**
   * Static animation presets
   */
  static presets = {
    slideLeft: { type: 'slide', direction: 'left', duration: 300 },
    slideRight: { type: 'slide', direction: 'right', duration: 300 },
    slideUp: { type: 'slide', direction: 'up', duration: 300 },
    slideDown: { type: 'slide', direction: 'down', duration: 300 },
    fadeIn: { type: 'fade', duration: 250 },
    fadeOut: { type: 'fade', duration: 250 },
    scaleIn: { type: 'scale', duration: 200 },
    scaleOut: { type: 'scale', duration: 200 },
  }
      ...
      case 'push':
        return direction === 'forward'
          ? {
              ...
              to: {
                initial: { transform: `translateX(${slideDistance})` },
 ...
 */
export function createProgrammaticNavigationPath(
...
export const ProgrammaticNavigationUtils = {
  /**
   * Validate navigation path segments
   */
  isValidPath(segments: string[]): boolean {
    return segments.every(segment => segment.length > 0)
  },

  /**
   * Normalize navigation path by removing empty segments
   */
  ...
  /**
   * Create navigation path from string
   */
  fromString(pathString: string): any {
    const segments = pathString.split('/').filter(Boolean)
    return createProgrammaticNavigationPath(segments)
  },

  /**
   * Calculate difference between two paths
   */
  calculateDifference(path1: any, path2: any): any {
    const segments1 = path1.segments || []
    const segments2 = path2.segments || []

    let commonPrefix: string[] = []
    for (let i = 0; i < Math.min(segments1.length, segments2.length); i++) {
      if (segments1[i] === segments2[i]) {
        commonPrefix.push(segments1[i])
      } else {
        break
      }
    }
    ...
    return {
      commonPrefix,
      removed: segments1.slice(commonPrefix.length),
      added: segments2.slice(commonPrefix.length),
    }
  },
   ...
   */
  findCommonPrefix(paths: any[]): string[] {
    if (paths.length === 0) return []
    if (paths.length === 1) return paths[0].segments || []

    const firstSegments = paths[0].segments || []
    let commonPrefix: string[] = []

    for (let i = 0; i < firstSegments.length; i++) {
      const segment = firstSegments[i]
      if (paths.every(path => (path.segments || [])[i] === segment)) {
