...
export const NavigationComponentValidation = {
  ...
  validateNavigationView(args: unknown[]): void {
    if (args.length === 0) {
      throw new NavigationValidationError('NavigationView component requires content', {
        component: 'NavigationView',
        suggestion: 'Add content: NavigationView(rootContent)',
        documentation: 'https://docs.tachui.dev/navigation/components/navigationview',
        example: {
    ...
    const [content] = args
    if (!content) {
      throw new NavigationValidationError('NavigationView content cannot be null or undefined', {
        component: 'NavigationView',
  ...
  validateNavigationStack(args: unknown[]): void {
    if (args.length === 0) {
      throw new NavigationValidationError('NavigationStack component requires content', {
        component: 'NavigationStack',
        suggestion: 'Add content: NavigationStack(rootContent)',
        documentation: 'https://docs.tachui.dev/navigation/components/navigationstack',
        example: {
    ...
    const [content] = args
    if (!content) {
      throw new NavigationValidationError('NavigationStack content cannot be null or undefined', {
        component: 'NavigationStack',
        suggestion: 'Provide valid content component',
        example: {
  ...
  validateNavigationLink(args: unknown[]): void {
    if (args.length === 0) {
      throw new NavigationValidationError('NavigationLink component requires destination or content', {
        component: 'NavigationLink',
        suggestion: 'Add destination: NavigationLink({ destination: "/path", label: "Link" })',
        documentation: 'https://docs.tachui.dev/navigation/components/navigationlink',
        example: {
          wrong: 'NavigationLink()',
          correct: 'NavigationLink({ destination: "/about", label: "About" })'
        }
      })
    }
    ...
    if (typeof props === 'string') {
      ...
      if (args.length < 2) {
        throw new NavigationValidationError('NavigationLink with string label requires destination', {
          component: 'NavigationLink',
      ...
      if (!propsObj.destination && !propsObj.content) {
        throw new NavigationValidationError('NavigationLink requires either destination or content property', {
          component: 'NavigationLink',
          suggestion: 'Provide destination or content',
          example: {
            wrong: 'NavigationLink({ label: "Link" })',
            correct: 'NavigationLink({ destination: "/path", label: "Link" })'
          }
        })
      }
    }
  ...
  validateTabView(args: unknown[]): void {
    if (args.length === 0) {
      throw new NavigationValidationError('TabView component requires tabs array', {
        component: 'TabView',
        suggestion: 'Add tabs: TabView([{ title: "Tab 1", content: Content1 }])',
        documentation: 'https://docs.tachui.dev/navigation/components/tabview',
        example: {
          wrong: 'TabView()',
    ...
    if (tabs.length === 0) {
      throw new NavigationValidationError('TabView must have at least one tab', {
        component: 'TabView',
    ...
    for (let i = 0; i < tabs.length; i++) {
      ...
      if (!tab || typeof tab !== 'object') {
        throw new NavigationValidationError(`Tab at index ${i} must be an object`, {
          component: 'TabView',
      ...
      if (!tab.title) {
        throw new NavigationValidationError(`Tab at index ${i} is missing required title property`, {
          component: 'TabView',
  ...
  validateSimpleTabView(args: unknown[]): void {
    if (args.length === 0) {
      throw new NavigationValidationError('SimpleTabView component requires tabs array', {
        component: 'SimpleTabView',
        suggestion: 'Add tabs: SimpleTabView([tabItem("Home", HomeView)])',
        documentation: 'https://docs.tachui.dev/navigation/components/simpletabview',
        example: {
          wrong: 'SimpleTabView()',
    ...
    const [tabs] = args
    if (!Array.isArray(tabs)) {
      throw new NavigationValidationError('SimpleTabView tabs must be an array', {
        component: 'SimpleTabView',
    ...
    if (tabs.length === 0) {
      throw new NavigationValidationError('SimpleTabView must have at least one tab', {
        component: 'SimpleTabView',
...
export async function registerNavigationValidators(): Promise<void> {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.info(`🔍 [@tachui/navigation] Registered ${validators.length} component validators`)
    }
  } catch (error) {
    if (process.env.NODE_ENV !== 'production') {
      console.warn('⚠️ [@tachui/navigation] Could not register validators with Core:', error)
    }
  }
}
