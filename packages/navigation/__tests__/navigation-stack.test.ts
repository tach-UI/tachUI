...
import { navigationTitle, navigationBarHidden } from '../src/navigation-modifiers'
...
describe('NavigationStack - SwiftUI Compatible Navigation System', () => {
  ...
  describe('Navigation Path Management', () => {
    it('initializes with navigation context', () => {
      ...
      expect((navStack as any).navigationContext.navigationId).toMatch(/^nav-stack-/)
    })
  ...
  describe('SwiftUI Compatibility', () => {
    ...
    it('works with NavigationLink children', () => {
      ...
      const rootWithLink = HTML.div({ children: [mockRootView, linkView] }).modifier.build()
  ...
  describe('Navigation Destination Support', () => {
    it('supports navigationDestination modifier', () => {
      const destinationView = HTML.div({ children: 'Destination' }).modifier.build()
      const rootWithDestination = navigationDestination(mockRootView, 'detail', () => destinationView)
      
      const navStack = NavigationStack(rootWithDestination)
      
      expect(navStack).toBeDefined()
      expect((rootWithDestination as any)._navigationDestinations).toBeDefined()
      expect((rootWithDestination as any)._navigationDestinations.detail).toBeDefined()
    })

    it('supports destination registration pattern', () => {
      const rootWithDestination = navigationDestination(mockRootView, 'detail', () => 
      ...
      expect(navStack).toBeDefined()
      expect((navStack as any)._navigationStack.registerDestination).toBeDefined()
    })
  })
  ...
  describe('Integration with Navigation System', () => {
    it('integrates with navigation modifiers', () => {
      const modifiedRoot = navigationBarHidden(navigationTitle(mockRootView, 'Home'), true)
