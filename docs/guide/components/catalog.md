...
| `Circle`, `Rectangle`, `RoundedRectangle`, `Ellipse`, `Capsule` | Shapes | Shapes that fill their frame; `.fill()`, `.stroke()`, `.strokeBorder()`, `.inset()`, `.trim()`, `.strokeStyle()` | `@tachui/primitives/shapes` |
