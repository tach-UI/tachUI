...
Unlike SwiftUI, TachUI uses direct modifier chaining on component instances:
...
TachUI uses a consistent direct-modifier pattern:
...
> **Important**: All TachUI modifiers must use the `.property()` pattern. There are no "direct" modifiers.
...
- **TachUI**: Complex styling uses direct modifier chaining
...
Always use direct modifier chaining for TachUI components:
