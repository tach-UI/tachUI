...
This first release ships the public type surface and the shared defaults, ahead of the runtime that lands across the rest of the `query-core` milestone. Declaring the types first is deliberate — `select` introduces the second type parameter on `QueryOptions` and makes `QueryResult`'s data parameter the projected `TData` rather than the cached `TRaw`, and the `status`/`fetchStatus` split defines what `idle` means, so both are effectively irreversible once anything depends on them.
